		...
		for _, member := range guild.Members {
			...
			if member.Nick == uName || member.User.Username == uName {
				...
				if len(args) == 3 {
					...
					return s.GuildMemberDeleteWithReason(guild.ID, member.User.ID, utility.ToString(args[3:], " "))
...
func newBanTimer(s *discordgo.Session, m *discordgo.Message) error {
	...
	if guild, err := servers.GetGuild(s, m); err != nil {
		...
		if len(args) < 3 {
			...
			// Returns an error if err is not nil
			if time, err := utility.StrToInt(args[2]); err != nil {
				return err
			} else {
