...
func CheckBotRole(s *discordgo.Session, g *discordgo.Guild) error {
	...
	// CommanD-Bot was not found with in guild //
	log.Println("CommanD-Bot was not found with in server")
	return nil
}
...
func MessageCreate(s *discordgo.Session, m *discordgo.MessageCreate) {
	...
	if _, ok := BotCommandKeys[args[0]]; ok != true {
		// Given command did not exist in map //
		log.Println("Command does not exists: ", args[0])
		return
	}
	...
	err = BotCommands[BotCommandKeys[args[0]]](s, m.Message, admin)
	if err != nil {
		log.Println(err)
	}
	return
